...
def plot_sinks_on_map(gdf_tracts, df_sinks, output_img_path, figsize=(10, 10), use_basemap=False):
    """
    ...
    sinks_geo_gdf = gpd.GeoDataFrame(
        df_sinks.copy(),
        geometry=[Point(lon, lat) for lat, lon in zip(df_sinks["lon"], df_sinks["lat"])],
        crs="EPSG:4326",
    ).to_crs(epsg=3857)
